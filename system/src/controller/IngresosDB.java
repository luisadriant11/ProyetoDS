...
public class IngresosDB {
    ...
    public void agregarClienteDB(Cliente cliente) {
        try {
            try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_cliente (Cli_Cedula, Cli_Nombre, Cli_Apellido, Cli_Direccion, Cli_Telefono, Cli_email, Cli_fecha_nacimiento) VALUES (?,?,?,?,?,?,?)")) {
    ...
    private int getIDVendedor(Vendedor vendedor) {
        ...
        try {
            ...
            try (PreparedStatement pst = conexion.prepareStatement("SELECT * FROM t_usuario WHERE cedula = ?")) {
    ...
    private int getIDCliente(String cedula) {
        ...
        try {
            logger.info("entra al nombre");
            try (PreparedStatement pst = conexion.prepareStatement("SELECT * FROM t_cliente WHERE Cli_Cedula = ?")) {
    ...
    public void ingresarVentaDB(Venta venta, Vendedor vendedor, String cedulaCliente) {
        try {
            try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_venta (Venta_Fecha, Venta_Subtotal, Venta_Total, Cliente_ID, Usua_ID, Venta_Cod) VALUES (?,?,?,?,?,?)")) {
    ...
    private void ingresoPagoEfectivo(Venta venta) {
        try {
            try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_pagoefectivo (PE_Fecha, PE_Subtotal, PE_Impuestos, Venta_ID) VALUES (?,?,?,?)")) {
                pst.setDate(1, Date.valueOf(new SimpleDateFormat("yyyy-MM-dd").format(venta.getFecha())));
                pst.setDouble(2, venta.getSubtotal());
    ...
    private void ingresoPagoCredito(Venta venta, FormaPago fpago) {
        try {
            try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_pagocredito (PC_Fecha, PC_Subtotal, PC_Impuestos, PC_PagoCorriente, PC_PagoDiferido, PC_MesesDiferidos, Venta_ID) VALUES (?,?,?,?,?,?,?)")) {
                pst.setDate(1, Date.valueOf(new SimpleDateFormat("yyyy-MM-dd").format(venta.getFecha())));
                pst.setDouble(2, venta.getSubtotal());
                pst.setDouble(3, venta.getSubtotal() * 0.12);
    ...
    private int getIDArt(String modelo) {
        ...
        try {
            logger.info("entra al nombre");
            try (PreparedStatement pst = conexion.prepareStatement("SELECT * FROM t_articulo WHERE Art_Modelo = ?")) {
    ...
    public void ingresarDetallesVenta(Venta venta, LinkedList<String> productosC) {
        ...
        productosC.forEach((p) -> {
            try {
                try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_detalle_venta(Venta_ID, Articulo_ID) VALUES (?,?)")) {
                    pst.setInt(1, idV);
    ...
    public void agregarArticuloDB(Articulo articulo) {        
        try {
            try (PreparedStatement pst = conexion.prepareStatement("INSERT INTO t_articulo (Art_Modelo, Art_Descripcion,Art_Precio, Art_Stock,Art_Categ, Art_Marca, Art_Color) VALUES (?,?,?,?,?,?,?)")) {
